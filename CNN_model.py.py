#-------------------------------------------------------------------
#                       Road and field classification for BilBerry
#-------------------------------------------------------------------

# if using Linux - (I ran on windows)
'''
#!/usr/bin/python
'''

# Installing Tensorflow
# pip install tensorflow

# Installing Keras
# pip install --upgrade keras

# Part 1 - Building the CNN

# Importing the Keras libraries and packages
from keras.models import Sequential
from keras.layers import Conv2D
from keras.layers import MaxPooling2D,Dropout
from keras.layers import Flatten
from keras.layers import Dense

def Road_field_classifier():
    # Initialising the CNN
    classifier = Sequential()
    
    # Step 1 - Convolution
    classifier.add(Conv2D(32, (3, 3), input_shape = (64, 64, 3), activation = 'relu'))
    
    # Step 2 - Pooling
    classifier.add(MaxPooling2D(pool_size = (2, 2)))
    classifier.add(Dropout(0.2))
    
    # Adding a second convolutional layer
    classifier.add(Conv2D(32, (3, 3), activation = 'relu'))
    classifier.add(MaxPooling2D(pool_size = (2, 2)))
    
    # Step 2 - Pooling
    classifier.add(MaxPooling2D(pool_size = (2, 2)))
    classifier.add(Dropout(0.2))
    
    # Step 2 - Pooling
    classifier.add(MaxPooling2D(pool_size = (2, 2)))
    classifier.add(Dropout(0.2))
    
    # Step 3 - Flattening
    classifier.add(Flatten())
    
    # Step 4 - Full connection
    classifier.add(Dense(units = 3672, activation = 'relu'))
    classifier.add(Dense(units = 1, activation = 'sigmoid'))
    return classifier


model = Road_field_classifier()
# Compiling the CNN
model.compile(optimizer = 'rmsprop', loss = 'binary_crossentropy', metrics = ['accuracy'])

# Image augmentation 
from keras.preprocessing.image import ImageDataGenerator
train_datagen = ImageDataGenerator(rescale = 1./255,
                                   shear_range = 0.2,
                                   zoom_range = 0.2,
                                   horizontal_flip = True)

# Image Scaling to facilitate optimization
test_datagen = ImageDataGenerator(rescale = 1./255)

# training set
training_set = train_datagen.flow_from_directory('data/training',
                                                 target_size = (64, 64),
                                                 batch_size = 5,
                                                 class_mode = 'binary')
# test set
test_set = test_datagen.flow_from_directory('data/test',
                                            target_size = (64, 64),
                                            batch_size = 5,
                                            class_mode = 'binary')


# fitting the model
model.fit_generator(training_set,
                         steps_per_epoch = 50,
                         epochs = 10,
                         validation_data = test_set,
                         validation_steps = 10)

 

# Finally evaluate this model on the test data 
results = model.evaluate_generator(test_set, steps=1) 

print('Final test accuracy:', (results[1]*100.0))

correct = 0
for i, n in enumerate(test_set.filenames):
    if n.startswith("r") and results[i][0] <= 0.5:
        correct += 1
    if n.startswith("f") and results[i][0] > 0.5:
        correct += 1

print("Correct:", correct, " Total: ", len(test_set.filenames))
print("Loss: ", results[0], "Accuracy: ", results[1])




# Serialize model to YAML
classifier_yaml = model.to_yaml()
with open("CNN_model.yaml", "w") as yaml_file:
    yaml_file.write(classifier_yaml)

# Serialize weights to HDF5
model.save_weights("CNN_model.h5")
print("Saved model to disk")
